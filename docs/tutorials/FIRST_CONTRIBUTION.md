...
Генератор создаст случайный 48-битный ID формата `PC-<год>-<12hex>` и один файл
в `candidates/`. ID не зависит от состояния других веток, поэтому независимые
PR не резервируют общий последовательный номер.
